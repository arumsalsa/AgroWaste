...
export function Sidebar({ mobileOpen = false, onClose }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { name: "Pesanan", path: "/seller/orders", badge: newOrdersCount > 0 ? newOrdersCount : undefined, icon: "M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" },
    ...
    >
      ...
      <div className="px-6 py-6 border-b border-seller-hairline/50 mb-4">
        <div className="flex items-center gap-3">
          <img src={avatarUrl || fallbackAvatar} alt="Avatar" className="w-10 h-10 object-cover rounded-full shadow-sm" />
          <div className="min-w-0 flex-1">
            ...
            <h2 className="text-xs font-bold text-seller-textprimary leading-tight mt-0.5 truncate" title={farmName}>{farmName}</h2>
          </div>
        </div>
      </div>
      ...
      <div className="p-6 border-t border-seller-hairline/50">
        <button onClick={handleLogout} className="flex items-center gap-3 text-seller-semred hover:text-red-700 font-semibold text-sm transition-colors w-full px-2">
