...
export default function InventoryPage() {
  ...
  const handleDeleteImage = async (mediaId: string) => {
    if (!editId) return;
    if (!confirm("Hapus gambar ini? Tindakan ini tidak bisa dibatalkan.")) return;
    setDeletingImageId(mediaId);
    setFormError(null);
    try {
      const res = await apiFetch(`/products/${editId}/images/${mediaId}`, { method: "DELETE" });
      const json = await res.json();
      if (res.ok && json.success) {
          ...
          prev.map((p) =>
            p.id !== editId
              ...
              : {
                  ...p,
                  media: updatedMedia,
                  image_url: updatedMedia.length > 0 ? (updatedMedia[0].original_url || updatedMedia[0].url || null) : null,
                  image_urls: updatedMedia.map((m) => m.original_url || m.url || "").filter(Boolean),
                }
          )
        );
      } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        const msg = json.message ?? (json.errors ? Object.values(json.errors).flat().join(" ") : "Gagal menyimpan produk.");
      ...
      if (images.length > 0) {
        ...
        try {
          ...
          const imgRes  = await apiFetch(`/products/${productId}/images`, { method: "POST", body: fd });
        ...
        setFormSuccess(
          formMode === "add" ? "Produk berhasil ditambahkan! Menunggu persetujuan admin." : "Produk berhasil diperbarui.",
        );
      }
  ...
  const field = (key: keyof ProductForm, label: string, type = "text", placeholder = "") => (
    <div>
      <label className="block text-xs font-semibold text-seller-textsecondary mb-1">{label}</label>
      <input
        type={type}
        value={form[key]}
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-seller-surfacewhite border border-seller-hairline p-5 rounded-2xl">
          ...
          <span className="text-[10px] font-bold text-seller-textsecondary block mb-1">Stok Tersedia</span>
          <h3 className="text-xl font-bold text-seller-textprimary font-tabular">
        ...
        <div className="bg-seller-primary p-5 rounded-2xl text-white shadow-lg shadow-seller-primary/20">
          ...
          <span className="text-[10px] font-bold text-white/90 block mb-1">Produk Aktif</span>
          <h3 className="text-xl font-bold text-white font-tabular">
            {loading ? "..." : `${products.filter((p) => p.status === "aktif").length} Aktif`}
          </h3>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            ...
            <tbody className="divide-y divide-seller-hairline bg-white">
              {loading && (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-seller-textsecondary text-xs animate-pulse">Memuat produk...</td></tr>
              )}
              {!loading && visible.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-seller-textsecondary text-xs">
                  {search ? "Tidak ada produk yang cocok." : "Belum ada produk. Klik \"Tambah Produk\" untuk mulai."}
                </td></tr>
              )}
              {!loading && visible.map((p) => {
                const { label, color, dot } = statusInfo(p.status);
                return (
                  <tr key={p.id} className="hover:bg-seller-warmbg/30 transition-colors">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded overflow-hidden bg-[#EAE6E1] flex items-center justify-center shrink-0 relative">
                          ...
                          <span className="text-[10px] font-bold text-seller-textsecondary">IMG</span>
                        </div>
                        <div>
                          <div className="font-bold text-seller-textprimary text-sm">{p.name}</div>
                          <div className="text-[10px] text-seller-textsecondary mt-0.5 uppercase tracking-wider">{p.jenis_ternak}</div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className={`flex items-center gap-1.5 text-[10px] font-bold tracking-wider ${color}`}>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-seller-surfacewhite w-full max-w-lg rounded-2xl border border-seller-hairline overflow-hidden animate-fade-in flex flex-col max-h-[92vh]">
            <div className="p-4 border-b border-seller-hairline flex justify-between items-center shrink-0">
              ...
              <button onClick={() => { setModalOpen(false); clearImages(); }} className="text-seller-textsecondary hover:text-seller-textprimary">
            ...
            <div className="p-5 overflow-y-auto space-y-4">
              ...
              {field("name", "Nama Produk *", "text", "Mis: Pupuk Kandang Sapi (Kering)")}

              <div className="grid grid-cols-2 gap-4">
                {/* Kategori */}
                <div>
                  <label className="block text-xs font-semibold text-seller-textsecondary mb-1">Kategori *</label>
                  <select
                ...
                {/* Jenis Ternak */}
                <div>
                  <label className="block text-xs font-semibold text-seller-textsecondary mb-1">Jenis Ternak *</label>
                  <select
                    value={form.jenis_ternak}
                    onChange={(e) => setForm((f) => ({ ...f, jenis_ternak: e.target.value }))}
                    className="w-full px-3 py-2 bg-seller-warmbg border border-seller-hairline rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-seller-primary"
                  >
                    {JENIS_OPTIONS.map((j) => (
              ...
              <div className="pt-2 border-t border-seller-hairline">
                <p className="text-[10px] font-bold text-seller-textsecondary uppercase tracking-wider mb-3">
                  Gambar Produk <span className="font-normal normal-case">(maks. 3 foto · 2MB/file · JPG/PNG)</span>
                </p>

                {/* Gambar Saat Ini (Mode Edit) */}
                {formMode === "edit" && editProductMedia.length > 0 && (
                  <div className="mb-4">
                    <span className="block text-xs font-semibold text-seller-textsecondary mb-2">Gambar Saat Ini ({editProductMedia.length}):</span>
                ...
                {/* Upload Gambar Baru */}
                {(formMode === "add" || editProductMedia.length + images.length < 3) && (
                  <label className="cursor-pointer block">
                    ...
                    <div className="border-2 border-dashed border-seller-hairline rounded-xl px-4 py-5 flex flex-col items-center gap-2 hover:border-seller-primary transition-colors">
                      <svg className="w-6 h-6 text-seller-textsecondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <span className="text-xs text-seller-textsecondary text-center leading-relaxed">
                        ...
                        <span className="block text-[10px] text-seller-textsecondary/70">
                          {3 - (formMode === "edit" ? editProductMedia.length : 0) - images.length} slot tersisa
                        </span>
                      </span>
                    </div>
                  </label>
                ...
                {imagePreviews.length > 0 && (
                  <div className="mt-3">
                    {formMode === "edit" && <span className="block text-xs font-semibold text-seller-textsecondary mb-2">Gambar Baru yang Dipilih:</span>}
                    <div className="flex gap-3 flex-wrap">
                ...
                {fileError && (
                  <p className="text-xs text-red-600 font-semibold mt-2">{fileError}</p>
                )}
              </div>
            </div>
            ...
            <div className="p-4 border-t border-seller-hairline flex justify-end gap-3 bg-[#F9F8F6] shrink-0">
              <button
                onClick={() => { setModalOpen(false); clearImages(); }}
                disabled={submitting}
              ...
              >
                {submitting ? submitLabel : formMode === "add" ? "Simpan Produk" : "Perbarui Produk"}
              </button>
            </div>
          </div>
        </div>
