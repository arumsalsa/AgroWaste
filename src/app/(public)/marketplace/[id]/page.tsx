...
interface Product {
  ...
  peternak_profile?: { nama_kandang?: string; nama_peternakan?: string; badge: string; user_id: string };
}
        ...
        apiFetch(`/products?sort=terbaru`)
          ...
          .then((simJson) => {
            if (simJson?.data?.data) {
              const list: Product[] = simJson.data.data.filter((item: Product) => item.id !== p.id);
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        setReviewMsg({ type: "error", text: json.message || "Gagal mengirim ulasan." });
      }
    } catch {
          ...
          <div className="lg:col-span-2">
            ...
            {activeTab === "deskripsi" ? (
              <div className="space-y-8 animate-fade-in">
                ...
                {(() => {
                  const nutrisiItems: [string, string][] = product.nutrisi && Array.isArray(product.nutrisi) && product.nutrisi.length > 0
                    ? product.nutrisi
                    : [["1.5%", "Nitrogen (N)"], ["0.8%", "Fosfor (P)"], ["1.2%", "Kalium (K)"], ["25%", "C-Organik"]];
                    ...
                    <div className="bg-[#FFF8F5] border border-[#E8E0D5] rounded-2xl p-6">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        {nutrisiItems.map(([val, label]) => (
                          <div key={label} className="bg-white border border-[#E8E0D5] p-4 rounded-xl text-center shadow-sm">
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                {canReview && (
                  <form onSubmit={handleReviewSubmit} className="bg-[#E6F5EC]/50 border border-[#009A44]/30 rounded-3xl p-6 sm:p-8">
                    <h3 className="font-bold text-[#111111] text-base mb-1">Berikan Ulasan Anda</h3>
                    <p className="text-xs text-[#555555] mb-4">Anda dapat memberikan ulasan karena telah menerima pesanan produk ini.</p>

                    {reviewMsg && (
                      <div className={`p-3 rounded-xl text-xs font-bold mb-4 ${
                        reviewMsg.type === "success" ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"
                      }`}>
                        {reviewMsg.text}
                      </div>
                    )}
                    ...
                    <div className="mb-4">
                      <label className="block text-xs font-bold text-[#555555] mb-2">Rating Produk *</label>
                      <div className="flex items-center gap-2">
                        {[1, 2, 3, 4, 5].map((star) => (
                          ...
                          >
                            <svg className={`w-7 h-7 ${star <= newRating ? "fill-amber-400" : "text-gray-300 fill-transparent"}`} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z" />
                            </svg>
                          </button>
                        ))}
                        <span className="text-xs font-bold text-[#009A44] ml-2">{newRating} / 5 Bintang</span>
                      </div>
                    </div>

                    <div className="mb-4">
                      <label className="block text-xs font-bold text-[#555555] mb-1">Komentar Ulasan</label>
                      <textarea
                        rows={3}
                        value={newComment}
                ...
                {reviews.length > 0 ? (
                  <div className="space-y-4">
                    {reviews.map((rev) => {
                      ...
                      return (
                        <div key={rev.id} className="bg-white border border-[#E8E0D5] rounded-2xl p-5 shadow-sm">
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <h4 className="text-xs sm:text-sm font-bold text-[#111111]">{userName}</h4>
                                <div className="flex items-center gap-1 text-[10px] text-[#555555]">
                                  <span>{new Date(rev.created_at).toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}</span>
                                </div>
                              </div>
                            </div>
                          ...
                          {rev.comment && (
                            <p className="text-xs sm:text-sm text-[#333333] leading-relaxed pl-12">{rev.comment}</p>
                          )}
                        </div>
                      );
                    })}
                  ...
                  !canReview && (
                    <div className="py-12 text-center border border-dashed border-[#E8E0D5] rounded-3xl bg-[#FBFAF7]/50 animate-fade-in w-full">
                      ...
                      <p className="text-xs text-[#555555]/75 mt-1">Lakukan pembelian dan konfirmasi penerimaan barang untuk memberikan ulasan!</p>
                    </div>
                  )
                )}
              </div>
                ...
                <Link href={`/sellers/${product.peternak_profile?.user_id}`} className="flex items-center justify-between border border-[#E8E0D5] rounded-xl p-3 mb-6 hover:border-[#009A44] transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#2C3930] flex items-center justify-center text-white text-sm font-bold">
                      {(product.peternak_profile?.nama_kandang || product.peternak_profile?.nama_peternakan || "P").charAt(0).toUpperCase()}
                    </div>
                    <div>
                      <div className="flex items-center gap-1">
                        <span className="text-xs font-bold text-[#111111]">{product.peternak_profile?.nama_kandang || product.peternak_profile?.nama_peternakan || "Peternak Organik"}</span>
          ...
          <div className="mt-20 border-t border-[#E8E0D5] pt-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {similarProducts.map((p) => {
                const formattedKab = p.kabupaten ? p.kabupaten.replace(/^Kabupaten\s+/i, "Kab. ") : "";
                ...
                return (
                  <EarthyCard
                    ...
                    title={p.name}
                    description={p.description ?? p.peternak_profile?.nama_kandang ?? p.peternak_profile?.nama_peternakan ?? "Produk organik berkualitas."}
                    imageUrl={p.image_url ? getProductImageUrl(p.image_url) : null}
                    imageFallbackIcon={Leaf}
                    badgeText={categoryBadge}
                    badgeDotColorClass="bg-land-accent"
                    locationText={formattedKab || undefined}
                    rating={Number(p.rating_avg) > 0 ? Number(p.rating_avg) : undefined}
