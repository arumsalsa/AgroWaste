...
export const StatCard: React.FC<StatCardProps> = ({ title, value, description, colorScheme }) => {
  ...
  return (
    <div className="bg-admin-surfacewhite border border-admin-hairline rounded-2xl p-6 relative overflow-hidden">
      ...
      <div className={`absolute right-0 top-0 w-24 h-24 rounded-bl-full -mr-4 -mt-4 ${arc}`} aria-hidden="true" />
      <span className="text-[10px] font-bold text-admin-textsecondary uppercase tracking-wider block mb-2">
        {title}
      </span>
      <div className={`text-3xl font-bold font-tabular ${valueColor}`}>{value}</div>
